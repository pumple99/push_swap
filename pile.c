...
static void	pile_only_top(t_stack *a)
{
	t_node	*t;
	t_node	*p;
...
void	pile_atop(t_stack *a, t_block blo)
{
	...
	if (blo.count == 2 && p->idx < a->top->idx)
		(swap(a) || ft_printf("sa\n"));
	else if (blo.count == 3 && a->arg_num == 3)
		pile_only_top(a);
	else if (blo.count == 3)
	{
		if (t->idx < p->prev->idx && p->prev->idx < p->idx)
...
static void	pile_abot3(t_stack *a, t_stack *b, int *e)
{
	t_node	*bot;
	t_node	*n;
	...
	if (n->next->idx < n->idx && n->idx < bot->idx)
		...
		(rro(a) || rro(a) || push(a, b, e) || rro(a) || swap(a) || \
		push(b, a, e) || ft_printf("rra\nrra\npb\nrra\nsa\npa\n"));
	else if (bot->idx < n->idx && n->idx < n->next->idx)
		(rro(a) || push(a, b, e) || rro(a) || rro(a) || swap(a) || \
		push(b, a, e) || ft_printf("rra\npb\nrra\nrra\nsa\npa\n"));
}
...
void	pile_abot(t_stack *a, t_stack *b, t_block blo, int *e)
{
	...
	else if (blo.count == 2)
	{
		if (n->idx < bot->idx)
			...
			(rro(a) || rro(a) || swap(a) || ft_printf("rra\nrra\nsa\n"));
	}
	else if (blo.count == 3)
		pile_abot3(a, b, e);
}
